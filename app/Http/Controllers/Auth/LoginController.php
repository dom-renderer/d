    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if (in_array('customer', $user->roles->pluck('name')->toArray())
            || in_array('vendor', $user->roles->pluck('name')->toArray())
            || in_array('technician', $user->roles->pluck('name')->toArray())) {
            session()->flush();
            return redirect()->to('login')->with('error', 'Your credentials are invalid');
        }
    ...
    {
        if (Auth::check()) {
            if (!in_array('admin', auth()->user()->roles()->pluck('name')->toArray()) && auth()->user()->can('jobs.index')) {
